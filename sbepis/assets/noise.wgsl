...
// ok now here's the part where i come and mess it all up
// never mind, copilot did it for me
...
fn worley(p: vec3f) -> WorleyResult {
    let cell: vec3i = vec3i(floor(p));
    let local: vec3f = fract(p);
    var minDist: f32 = 2.0;
	var minDist2: f32 = 2.0;
    var cellID: vec3i = vec3i(0);
    for (var x = -1; x <= 1; x++) {
        for (var y = -1; y <= 1; y++) {
            for (var z = -1; z <= 1; z++) {
                let neighbor: vec3i = cell + vec3i(x, y, z);
