...
fn update_walk_velocity_walk(players: Query<Entity, With<Walking>>, mut commands: Commands) {
...
fn update_walk_velocity(
    execute: On<DIExecute>,
    mut movement: Query<(&mut Movement, &Velocity, &Transform, &WalkDI, Has<Grounded>)>,
    walk_settings: Res<PlayerWalkSettings>,
    time: Res<Time>,
) -> Result {
    let (mut movement, velocity, transform, di, grounded) = movement.get_mut(execute.entity)?;
    // Set up vectors
